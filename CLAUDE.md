...
当前工作目录下包含以下结构：
- `/drafts`: 存放流水线自动生成的知识点草稿 (.md)。初始为空，由 content-architect 在 Step 4 创建。
- `/exercises`: 存放配套的习题库与标准答案。
- `/finished_outputs`: 存放最终满分通过的教材。
- `content-architect`, `evidence-based-student`, `faithfulness-examiner`: 子智能体规则。
...
- **考官和建筑师保留上下文**：同一文件循环期间，faithfulness-examiner 和 content-architect 的每次调用保持上下文连续（考官记住自己命的题和审过的答卷；建筑师记住上一轮的草稿和知识缺陷）。
- **学生每次全新启动**：evidence-based-student 每轮调用均全新启动，不携带之前上下文。预读协议保证它每次从文件系统重新加载全部知识。

### Step 1: 考官自主命题
- 全新加载 `faithfulness-examiner` subagent。
- 提供给考官：
  - 下一个文件序号（如 "03"）
  - **全部前序已完成文件的完整路径**
...
- **考官任务**：
  ...
  4. 输出 `## [Next_Knowledge_Point]` + `## [Blind_Exam]` + `## [Answer_Key]`
- **终止信号**：若考官输出 `CHAPTER_COMPLETE`，总场控执行章节闭合流程（见下方"章节闭合与续章"）。

### Step 2: 盲测与取证
- **Step 1 成功后总场控暂存**：提取考官输出的 `## [Next_Knowledge_Point]`（知识点名称）、`## [Blind_Exam]`（试卷原文）、`## [Answer_Key]`（标准答案）。三者在整个文件循环期间保持不变。
- 全新加载 `evidence-based-student` subagent。
- 提供学生阅读清单：
...
### Step 3: 双重审计与决断
- 全新加载 `faithfulness-examiner` subagent。**传入完整上下文使其"延续"**：
...
### Step 4: 靶向重构 / 创建
- 全新加载 `content-architect` subagent。**传入完整上下文使其"延续"**：
  - 知识点名称/序号
...
3. 考官扫描 `exercises/` 中所有习题，与已完成章节覆盖的知识面进行比对，自主判断：
   - **存在未被覆盖的习题** → 考官自主命名新章节，输出新章节的 `## [Next_Knowledge_Point]` + `## [Blind_Exam]` + `## [Answer_Key]`。总场控将新章节追加到 `pipeline-state.json`（`status: in_progress`，`files_completed: []`），流水线继续。
...
## 环境约束
- 工作目录为当前项目根目录（即 CLAUDE.md 所在目录），所有 git 命令直接执行（如 `git status`、`git add`、`git commit`），**禁止**使用 `cd` 前缀。多余的 cd 会触发不必要的权限审核。
