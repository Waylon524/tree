...
- `/source_materials`: PaddleOCR + Archivist 处理后的结构化 Markdown 资料，是 examiner 命题与章节发现的唯一资料入口。
...
流水线不使用预定义知识点清单、章节清单或外部习题库。用户上传资料后，引擎先将原始资料转为结构化 `source_materials/<collection>/*.md`。随后 examiner 基于这些结构化资料自主发现章节、确定下一个知识点并命题；student 只依据前序成品与当前草稿作答；examiner 审计答卷；writer 按缺陷报告创建或优化草稿。循环直到所有结构化资料覆盖完毕。
...
5. `tree-run run` 从 `source_materials/` 发现章节并启动以考促写循环。
...
- 提供给 examiner：
  - 下一个文件序号。
  - 当前章节 `source_collection` 下的全部结构化资料路径与全文。
  - 全部前序已完成文件路径与全文。
...
- student 只获得：
  - 前序已完成文件。
  - 当前知识点草稿（若存在）。
  - examiner 的 `[Blind_Exam]` 与 `[Student_Instructions]`。
...
- examiner 获得：
  ...
  - 前序已完成文件路径与全文。
  - 当前章节结构化资料路径与全文，用于确认 writer 应覆盖的知识范围。
...
- writer 获得：
  ...
  - 当前草稿全文（若存在）。
  - 前序已完成文件路径与全文。
  - examiner 的 `[Architect_Instructions]`。
...
2. examiner 扫描 `source_materials/` 下所有 collection，与 `pipeline-state.json` 中已完成章节比对。
