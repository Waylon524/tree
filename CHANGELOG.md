...
- 修复 Windows 文本模式把 NodeRun 输出中的 LF 转换为 CRLF、导致落盘文件哈希与 Knowledge Ledger 不一致的问题；统一文本写入现在直接持久化确定性的 UTF-8 字节，已成功生成的节点不会再被误判为未覆盖、重复生成带节点后缀的同一果实，或因后续无效重试变成枯果。

## 1.1.0 - 2026-07-17

### Changed

- 本地 embedding 请求默认超时由 60 秒提高到 300 秒，并在照料页高级设置中开放 `EMBED_REQUEST_TIMEOUT_SEC`；同一文档沿用现有 MTU 和物理段 token 规则，但改为逐段请求，让每个既有物理段独立获得完整超时预算，远程 embedding 端点继续批量请求。

### Fixed

- 修复 Windows CPU 较慢时，长 embedding 段或同一文档的多个既有物理段共享 60 秒总预算而在播种阶段中途失败的问题；超时错误现在包含段序号、字符数、端点、模型、实际超时和恢复建议，索引仍只在全部向量成功后原子替换。
...
- 为收获页果实状态和生长页七阶段增加即时说明层，支持鼠标悬停、键盘聚焦、Escape 关闭和中英文功能说明；固定镜头算法与推荐理由兼容映射增加前端单元测试。
...
- README 现在明确对应 `0.4.0`，统一使用“生长 / Grow”“休眠 / Rest”和当前果园按钮名称，把生长页实现细节精简为面向用户的运行、恢复与隐私说明，并同步最终发布资产的下载链接与 SHA-256。
- 生长页将“生长”和“休眠”合并为标题右侧的单一状态按钮：休眠时启动生长，运行或启动时切换为休眠；果园卡片移除占用体积，观察与更多操作各占一半卡片宽度。
- 重整照料页视觉层级：隐私提示与语言切换合并到页头，LLM、OCR、运行参数使用一致的分区卡片、输入焦点样式与响应式间距，高级设置和保存操作保持清晰分离。
- 重构收获页三栏布局：移除重复的果实/枝条统计并缩窄左栏，将树冠滑杆悬浮到 3D 画布顶部；右侧检查器改为主信息、推荐、学习操作、后续筛选和关系定义分区。
- 学习推荐理由由后端英文展示文本改为结构化理由代码，前端根据当前语言渲染，并兼容已知旧版理由且拒绝展示未知英文原文。
...
- 修复收获页果实阶段悬停说明受侧栏 stacking context 与 3D WebGL 画布遮挡的问题；说明层现在通过窗口级 Portal 渲染，并保留鼠标、键盘与 Escape 交互。
- 修复收获页树根附近额外圆柱草地补丁产生圆形破碎色块或闪烁的问题；移除该叠层并保留单一连续地面。
- 修复点击果实按当前距离比例缩放、连续选择后构图不一致的问题；现在保留观察方向并始终移动到固定距离，摄像机与节点重合时使用稳定回退方向。
