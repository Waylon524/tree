...
| 项目 | 当前实现 |
| --- | --- |
| 桌面架构 | Tauri 原生壳加载 React 前端，并为当前项目静默启动一个本机、令牌保护的 Python engine sidecar；Windows 源码构建不会额外打开控制台窗口。 |
| 桌面图标 | 安装包图标与界面左上角 `Tree` 品牌统一使用“知识树从书页生长”的新图标，并为 macOS、Windows 和常见桌面尺寸提供原生资源。 |
...
当前发布版本：[v1.0.0](https://github.com/Waylon524/tree/releases/tag/v1.0.0)

| 平台 | 安装包 | 说明 |
| --- | --- | --- |
| macOS Apple Silicon | [TREE_1.0.0_macos.dmg](https://github.com/Waylon524/tree/releases/download/v1.0.0/TREE_1.0.0_macos.dmg) | 已使用 Developer ID 签名、公证并 stapled。 |
| Windows x64 | [TREE_1.0.0_x64-setup.exe](https://github.com/Waylon524/tree/releases/download/v1.0.0/TREE_1.0.0_x64-setup.exe) | NSIS 安装包。Windows 可能因新应用信誉不足显示 SmartScreen 提醒。 |
| Windows x64 | [TREE_1.0.0_x64_en-US.msi](https://github.com/Waylon524/tree/releases/download/v1.0.0/TREE_1.0.0_x64_en-US.msi) | MSI 安装包，适合偏好 Windows Installer 的场景。 |

SHA-256 校验值：

- `TREE_1.0.0_macos.dmg`: `8cc12024a2e96b2119bf0b3015c366c2378500174735a4b3b026f29e46dd90c5`
- `TREE_1.0.0_x64-setup.exe`: `d268e30ebd7734431d00a0ce7f71953a265aeee16de0c87935041209a8f01a51`
- `TREE_1.0.0_x64_en-US.msi`: `d385691d7942c6df4d9431e098bda735414142ce798455cc7ca631b419609e92`

Release 同时提供可直接校验的 [macOS 清单](https://github.com/Waylon524/tree/releases/download/v1.0.0/SHA256SUMS-macos.txt) 和 [Windows 清单](https://github.com/Waylon524/tree/releases/download/v1.0.0/SHA256SUMS-windows.txt)。
...
照料页用于准备项目和配置运行环境。LLM、OCR 与运行参数按独立区域组织，常用配置优先展示，高级参数、存储位置和 Agent 提示词保持折叠，减少日常配置时的视觉干扰。
...
生长页是流水线控制台。标题右侧可即时切换 NodeRun 标准/快速模式，并在其右侧保留一个状态相关操作：果树休眠时显示 **生长 / Grow**，运行或启动时显示 **休眠 / Rest**。模式切换点击后立即保存；标准模式保留多智能体审核循环，快速模式每个新节点只调用一次 Fast Writer，已经启动的节点保持原模式。点击生长后，TREE 会依次推进：
...
NodeRun 默认使用标准模式，由 Examiner、Student 与 Writer 循环审核和修订；Examiner 交给 Writer 的节点范围和目标节点仍会严格校验，未提供的公式、推导、组织等可选建议由程序补入安全默认值，避免纯格式差异中断生成。快速模式为每个新节点调用一次 Fast Writer，同时保留节点边界、来源追溯、原子发布和 finished RAG 索引。模式在节点启动时固定，运行中切换只影响之后启动的节点；“重新生长”会清除该节点的旧试卷、草稿和迭代状态，从头生成。
...
原来的 DAG 页面现在是学习入口。3D 果树用“小果实、青果、转色、将熟、熟透、已收、枯果”统一表达从先修未完成、等待生成、正在生成、已经生成到推荐阅读、已读和失败的完整状态；鼠标悬停或键盘聚焦左侧分类可即时查看状态含义，说明层挂在窗口最顶层，不会被中间的 3D 画布遮挡。左侧只保留分类与更新时间，树冠大小控制悬浮在画布顶部；点击任意果实都会沿当前观察方向移动到统一镜头距离，适配和复位仍可恢复全树视图。

右侧检查器把成熟度、编号、标题、推荐理由、摘要和“开始学习”集中在主卡片，并把后续筛选、先修、后继与定义分区展示。推荐规则是确定性的：优先推荐“前置节点已经读完、自己还没读”的节点；后端只返回稳定的理由代码，由界面按当前语言生成说明。推荐只是提示，你仍然可以自行选择任意已生成节点阅读。
