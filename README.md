...
原来的 DAG 页面现在是学习入口。3D 果树用“小果实、青果、转色、将熟、熟透、已收、枯果”统一表达从先修未完成、等待生成、正在生成、已经生成到推荐阅读、已读和失败的完整状态；鼠标悬停或键盘聚焦左侧分类可即时查看状态含义。左侧只保留分类与更新时间，树冠大小控制悬浮在画布顶部；点击任意果实都会沿当前观察方向移动到统一镜头距离，适配和复位仍可恢复全树视图。
