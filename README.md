...
TREE 有两个可选依赖组（extras）：

- `rag`：可移植的 RAG 依赖（向量库 + 模型缓存发现），所有平台（含原生 Windows）都能装。
- `local-embed`：本地托管 embedding server 的原生依赖（`llama-cpp-python` 等），在 macOS /
  Linux / WSL2 上可用；原生 Windows 不装它，改连外部 embedding 端点（见 [Windows](#windows)）。
...
pipx install "tree-engine[rag,local-embed] @ git+https://github.com/Waylon524/tree.git"
```
...
Windows 有两条路：**原生 Windows（推荐普通用户）** 用外部 embedding 端点；**WSL2 Ubuntu**
保留零配置的本地 embedding（和 macOS / Linux 完全一致）。

TREE 的核心流程在原生 Windows 上可以直接跑，唯一不能本地托管的是内置 embedding server
（依赖 `llama-cpp-python` 等在 Windows 上安装不稳定的原生组件）。所以原生 Windows 不安装
`local-embed`，改为连接一个外部的 OpenAI 兼容 embedding 端点，最简单的是本机 Ollama。

#### 原生 Windows（PowerShell）

先安装 Python 3.12+、Git 和 pipx，再安装 TREE（只装可移植的 `[rag]`，不装 `local-embed`）：
...
然后安装 [Ollama for Windows](https://ollama.com/download)，拉取与 TREE 默认同族的
Qwen3 embedding 模型并确认 Ollama 在运行（默认监听 `http://localhost:11434`）：
...
最后让 TREE 指向 Ollama 的 OpenAI 兼容端点。把下面三个环境变量加入 PowerShell（当前会话用
`$env:`，永久生效用 `setx` 后重开终端）：

```powershell
$env:EMBED_API_URL = "http://localhost:11434"
$env:EMBED_MODEL = "qwen3-embedding:0.6b"
$env:EMBED_AUTO_START = "false"
```

`EMBED_AUTO_START=false` 告诉 TREE「embedding 端点我自己提供」，不要尝试托管本地 server。
之后就和其它平台一样：
...
`tre doctor` 里 `local embed` 一栏显示 `missing` 是正常的（原生 Windows 不需要它），只要
`embedding server` 显示 `external`、Ollama 正在运行即可。

#### WSL2 Ubuntu（零配置本地 embedding）

如果希望和 macOS / Linux 一样由 TREE 自动下载模型并托管本地 embedding server，使用 WSL2。
...
然后在 WSL2 Ubuntu 终端中安装 Git、Python、pipx，并用 pipx 安装 TREE（含 `local-embed`）：
...
TREE 的 RAG 有两种 embedding 来源：

- **本地托管（macOS / Linux / WSL2，需 `local-embed` extra）**：TREE 自带 Qwen3 Embedding
  GGUF server，默认 OpenAI 兼容端点为 `http://localhost:8788/v1/embeddings`。
- **外部端点（任意平台，包括原生 Windows）**：把 `EMBED_API_URL` 指向任意 OpenAI 兼容
  embeddings endpoint（如 Ollama），并设 `EMBED_AUTO_START=false`，TREE 不再托管本地 server。

### 本地托管

项目内置 Qwen3 Embedding GGUF server，默认使用更轻量的 0.6B Q8 模型。普通用户不需要手动启动：进入 `TREE>` 后，`/run` 会在需要 RAG 时自动检查模型、下载缺失模型并启动本地 embedding server；`/quit` 会停止 TREE 托管的 engine 和 embedding server。本地托管需要安装 `local-embed` extra（`llama-cpp-python` / `fastapi` / `uvicorn`）。
...
完整端到端运行需要可移植的 `[rag]` extra（向量库等）。

macOS / Linux / WSL2（含本地 embedding server）：

```bash
pip install -e ".[rag,local-embed,dev]"
```

原生 Windows（连接外部 embedding 端点，不装 `local-embed`）：
...
如果在 `/run` 或 `tre embedding start` 时看到提示 `local-embed` extra 缺失，说明 TREE 想托管
本地 embedding server，但没装 `llama-cpp-python` / `fastapi` / `uvicorn`。两种解决方式：

- 安装本地 server 依赖（macOS / Linux / WSL2）：`pip install 'tree-engine[local-embed]'`。
- 或改用外部 embedding 端点（推荐原生 Windows）：设置 `EMBED_API_URL` + `EMBED_AUTO_START=false`，
  见 [Embedding 服务 → 外部端点](#外部端点含原生-windows)。
