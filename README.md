...
T.R.E.E.（Textbook Refinement & Enhancement Engine）是一套资料驱动的自动化教材生成流水线。用户把课件、习题、讲义、图片或文本资料放入 `raw_materials/` 后，引擎会先完成 OCR、结构化整理和本地向量化入库，再通过“以考促写”的循环持续生成教材内容。
...
T.R.E.E. 默认使用 `Qwen/Qwen3-Embedding-4B-GGUF` 中的 `Qwen3-Embedding-4B-Q8_0.gguf`。首次启动 embedding server 时会自动下载模型，文件大小约 4.3 GB。
...
```text
https://github.com/Waylon524/T.R.E.E..git
```

GitHub 页面显示的仓库名是 `T.R.E.E.`。
...
T.R.E.E. uses `Qwen3-Embedding-4B-Q8_0.gguf` from `Qwen/Qwen3-Embedding-4B-GGUF` by default. The model is downloaded automatically on the first embedding server start. The file is about 4.3 GB.
...
The remote URL contains a trailing dot:

```text
https://github.com/Waylon524/T.R.E.E..git
```

The GitHub repository name is displayed as `T.R.E.E.`.
